...
function ensureCallable(fn, name) {
  if (typeof fn !== "function") {
    const err = new Error(`${name} is not a function (type=${typeof fn}). Check require path / module.exports.`);
  ...
  try {
    ...
    if (Array.isArray(batch) && batch.length > 0) {
      ...
      try {
        ...
        return res.status(500).json({ error: "Runner failed", detail: String(e && e.message ? e.message : e) });
      }
      ...
      for (const result of runnerResults) {
        ...
        if (lowErr.includes("compile") || lowErr.includes("javac") || lowErr.includes("error:")) {
    ...
    if (problem) {
      ...
      const referencePath = path.join(process.cwd(), "reference", `${problem}.${ext}`);
    ...
    console.error("Execution handler error:", err && err.stack ? err.stack : err);
