...
class JobQueue {
    ...
    async enqueue(lang, code, sessionId, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                reject(new Error(`Job ${jobId} timed out after ${options.timeout || 30000}ms`));
    ...
    async streamOutput(streamChannel, data) {
        await this.publisher.publish(streamChannel, JSON.stringify({ type: 'output', data }));
    }
    ...
    async streamDone(streamChannel) {
        await this.publisher.publish(streamChannel, JSON.stringify({ type: 'done' }));
    }
