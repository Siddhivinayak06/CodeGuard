...
const handleConnection = (ws) => {
    ...
    const startSession = (newLang) => {
        ...
        if (newLang === "python") {
            setTimeout(() => {
                pythonProcess = dockerService.execPython(containerName,
            ...
            setTimeout(() => {
                cProcess = dockerService.execC(containerName,
                    (data) => {
            ...
            setTimeout(() => {
                javaProcess = dockerService.execJava(containerName,
                    (data) => safeSend(ws, data.toString()),
    ...
    ws.on("message", (msg) => {
        ...
        if (parsed.type === "lang") {
            ...
            // Multi-file support
            fileBuffer.push({
                name: parsed.filename || "main" + (lang === "python" ? ".py" : lang === "java" ? ".java" : ".c"),
                content: parsed.data || "",
