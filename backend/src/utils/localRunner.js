...
function compileLocally(code, lang, workDir) {
    ...
    return Promise.resolve({ stdout: '', stderr: 'Unsupported language', exitCode: 1 });
}
...
function execTestCaseLocally(tc, lang, workDir) {
    ...
    if (lang === 'python' || lang === 'py') {
        ...
        const classFiles = fs.readdirSync(workDir).filter((f) => f.endsWith('.class'));
        let mainClass = 'Main';
