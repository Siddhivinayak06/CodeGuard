...
export async function POST(req: Request) {
  try {
    const { submissionId, status, output, marks_obtained, execution_details, code } = await req.json();
    ...
    if (shouldUpdateCode) {
      ...
      console.log(`Keeping existing marks (${existingMarks}) as they are higher than new marks (${newMarks})`);
    }
