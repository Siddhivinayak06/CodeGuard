...
export function ExportControls({ schedules }: ExportControlsProps) {
  ...
  const exportPDF = () => {
    ...
    doc.text("Yearly Practical Schedule", pageWidth / 2, 30, {
      align: "center",
    });
    ...
    doc.text(`Academic Year ${currentYear}`, pageWidth / 2, 42, {
      align: "center",
    });
    ...
    const uniquePracticals = new Set(schedules.map((s) => s.practicals?.title))
      .size;
    const uniqueBatches = new Set(
      schedules.map((s) => s.batch_name).filter(Boolean),
    ).size;
    ...
    months.forEach((month, monthIndex) => {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        `${monthSchedules.length} session${monthSchedules.length > 1 ? "s" : ""}`,
        pageWidth - 15,
        13,
        { align: "right" },
      );
      ...
      autoTable(doc, {
        head: [
          ["#", "Date", "Time", "Practical", "Language", "Batch", "Faculty"],
        ],
        body: tableData,
        startY: 28,
        theme: "grid",
        ...
        didDrawPage: (data) => {
          ...
          doc.text("CodeGuard - Practical Schedule", 15, pageHeight - 8);
        },
      });
    });
  ...
  const exportExcel = () => {
    ...
    const groupedByMonth = groupByMonth(schedules);
    const months = Object.keys(groupedByMonth);
    const uniquePracticals = [
      ...new Set(schedules.map((s) => s.practicals?.title).filter(Boolean)),
    ];
    const uniqueBatches = [
      ...new Set(schedules.map((s) => s.batch_name).filter(Boolean)),
    ];
    const uniqueFaculty = [
      ...new Set(
        schedules
          .map((s) => getFacultyName(s.faculty))
          .filter((f) => f !== "TBA"),
      ),
    ];
    ...
    const summaryData = [
      ...
      [
        "Unique Practicals",
        uniquePracticals.length,
        "",
        uniquePracticals.slice(0, 3).join(", ") +
          (uniquePracticals.length > 3 ? "..." : ""),
      ],
      ...
      [
        ...
        "",
        uniqueFaculty.slice(0, 3).join(", ") +
          (uniqueFaculty.length > 3 ? "..." : ""),
      ],
      ["", "", "", ""],
    ...
    months.forEach((month) => {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (daySchedules.length > 0) {
          const sessionInfo = daySchedules
            .map(
              (s) =>
    ...
    uniqueBatches.forEach((batch) => {
      ...
      batchData.push([
        `📌 ${batch}`,
        `${batchSchedules.length} sessions`,
        "",
        "",
        "",
        "",
    ...
    uniqueFaculty.forEach((faculty) => {
      const facultySchedules = sorted.filter(
        (s) => getFacultyName(s.faculty) === faculty,
      );
      const facultyBatches = [
        ...new Set(facultySchedules.map((s) => s.batch_name).filter(Boolean)),
      ];
      const facultyPracticals = [
        ...new Set(
          facultySchedules.map((s) => s.practicals?.title).filter(Boolean),
        ),
      ];
      const upcomingSessions = facultySchedules.filter(
      ...
      facultyData.push([
        ...
        facultyPracticals.slice(0, 3).join(", ") +
          (facultyPracticals.length > 3 ? "..." : ""),
        nextSession,
      ]);
    });
