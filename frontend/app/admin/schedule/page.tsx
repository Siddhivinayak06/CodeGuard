          ...
          {!loading && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-200 dark:border-gray-800 animate-fadeIn animation-delay-200">
              ...
              <StatCard
                label="Upcoming"
                value={
                  schedules.filter((s: any) => new Date(s.date) > new Date())
                    .length
                }
                color="green"
...
function StatCard({
  ...
  const colorClasses = {
    ...
    purple:
      "bg-purple-50/50 dark:bg-purple-900/10 border-purple-100 dark:border-purple-800 text-purple-700 dark:text-purple-300",
    green:
      "bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300",
  };

  return (
    <div
      className={`relative rounded-xl border p-5 transition-all hover:scale-[1.02] hover:shadow-md cursor-default ${colorClasses[color as keyof typeof colorClasses]}`}
    >
